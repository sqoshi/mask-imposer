...
import cv2
from landmarks_predictor.image import Image
...
class MaskImage(Image):
    ...
    def resized(
        ...
        """
        x_diff = abs(
            self._point_map.get_left_point().x - self._point_map.get_right_point().x
        )
        y_diff = abs(
            self._point_map.get_bottom_point().y - self._point_map.get_top_point().y
        )
